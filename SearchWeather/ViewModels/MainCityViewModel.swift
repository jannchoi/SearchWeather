...
final class MainCityViewModel {
    ...
    let selectedCity = BehaviorRelay(value: UserDefaultsManager.city)
    ...
    func transform(input: Input) -> Output {
        let id = selectedCity.value.id
        let inputButtonTapped = input.reloadDataTrigger.map{
            id}
        Observable.merge(selectedCity.map{$0.id},inputButtonTapped)
            .bind(with: self) { owner, id in
                owner.getWeather([id])
            }.disposed(by: disposeBag)
        
        NotificationCenter.default.rx.notification(Notification.Name("city")).bind(with: self) { owner, notification in
                guard let item = notification.object as? SelectedCity else {return}
            UserDefaultsManager.city = item
            owner.selectedCity.accept(item)
        }.disposed(by: disposeBag)
    ...
    private func mappingCityWeather()  { // 서버에서 얻은 데이터와 cityinfo데이터를 cityweather로 매핑
        ...
        let selectedCity = selectedCity.value
